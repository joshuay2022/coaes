...
CoAes is a proof search tactic for Lean 4, based on
[Aesop](https://github.com/leanprover-community/aesop) (v4.26.0). It provides
the tactics `coaes` and `coaes?` (the latter prints the proof it found as a
`Try this` suggestion, in standard Lean format with indentation and `·`
bullets).

CoAes extends Aesop with:

- **Unfolding with highest priority.** `coaes (defs := [f, g])` registers `f`
  and `g` as unfold rules that run before every other rule. Unlike Aesop's
  `unfold` rules, recursive definitions are allowed: each definition is
  unfolded at most once per goal, so unfolding cannot loop.
  `coaes (no_unfold := [h])` blocks `h` from being unfolded (whether it was
  registered via `defs`, `add norm unfold` or the `@[coaes]` attribute).
- **Standalone closing tactics.** `simp`, `simp_all`, `omega`, `decide` and
  `grind` are tried on every goal before any other rule except `unfold` — in
  particular before `intro`. Each succeeds only if it closes the goal, and each
  is recorded in the `coaes?` script as its bare name. They can be disabled
  individually, e.g. `coaes (erase CoAes.BuiltinRules.closeGrind)`.
- **Simplified scripts.** `coaes?` prefers to record simple tactics: `simp` /
  `simp_all` (listing only premises outside the default simp set) instead of
...
(e.g. Mathlib's `linarith`) can be registered like this:

```lean
@[coaes norm -145 tactic]
meta def closeLinarith : CoAes.RuleTac :=
  CoAes.RuleTac.closingTacticSyntax $ Lean.Unhygienic.run `(tactic| linarith)
```

The remainder of this README is Aesop's documentation, adapted to the CoAes
names. In essence, CoAes works like this:

- As with `simp`, you tag a (large) collection of definitions with the
  `@[coaes]` attribute, registering them as CoAes _rules_. Rules can be
  arbitrary tactics. We provide convenient ways to create common types of rules,
  e.g. rules which apply a lemma.
- CoAes takes these rules and tries to apply each of them to the initial goal.
  If a rule succeeds and generates subgoals, CoAes recursively applies the rules
  to these subgoals, building a _search tree_.
- The search tree is explored in a _best-first_ manner. You can mark rules as
  more or less likely to be useful. Based on this information, CoAes prioritises
...
- Before any rules are applied to a goal, it is _normalised_, using a special
  ...
  account by CoAes.
- Rules can be marked as _safe_ to optimise CoAes's performance. A safe rule is
  applied eagerly and is never backtracked. For example, CoAes's built-in rules
  safely split a goal `P ∧ Q` into goals for `P` and `Q`. After this split, the
  original goal `P ∧ Q` is never revisited.
- CoAes provides a set of built-in rules which perform logical operations (e.g.
  case-split on hypotheses `P ∨ Q`) and some other straightforward deductions.
- CoAes uses indexing methods similar to those of `simp` and other Lean tactics.
  This means it should remain reasonably fast even with a large rule set.
- When called as `coaes?`, CoAes prints a tactic script that proves the goal,
  similar to `simp?`. This way you can avoid the performance penalty of running
  CoAes all the time. However, the script generation is currently not fully
  reliable, so you may have to adjust the generated script.

CoAes is suitable for two main use cases:

- General-purpose automation, where CoAes is used to dispatch 'trivial' goals.
  By registering enough lemmas as CoAes rules, you can turn CoAes into a much
  more powerful `simp`.
- Special-purpose automation, where specific CoAes rule sets are built to
  address a certain class of goals. Mathlib tactics such as `measurability`
  and `continuity` are implemented by CoAes.
...
There's also [a paper about CoAes](https://zenodo.org/record/7430233) which
...
## Adding CoAes to Your Project

To use CoAes in a Lean 4 project, first add this package as a dependency. In
your `lakefile.lean`, add

```lean
require coaes from git "https://github.com/leanprover-community/coaes"
```

You also need to make sure that your `lean-toolchain` file contains the same
version of Lean 4 as CoAes's, and that your versions of CoAes's dependencies
...
To get you started, I'll explain CoAes's major concepts with a series of
...
When CoAes first encounters a goal, it normalises it by running a customisable
set of normalisation rules. One such normalisation rule effectively runs
`simp_all`, so CoAes automatically takes `simp` lemmas into account.

Now we define the `NonEmpty` predicate on `MyList`:

``` lean
@[coaes safe [constructors, cases]]
inductive NonEmpty : MyList α → Prop
  | cons : NonEmpty (cons x xs)
```

Here we see the first proper CoAes feature: we use the **`@[coaes]`** attribute
to construct two CoAes rules related to the `NonEmpty` type. These rules are
added to a global rule set. When CoAes searches for a proof, it systematically
...
proved when CoAes applies a rule that generates no subgoals.

In general, rules can be arbitrary tactics. But since you probably don't want to
write a tactic for every rule, the `coaes` attribute provides several **rule
...
``` lean
@[coaes unsafe 50% apply]
theorem nonEmpty_append₁ {xs : MyList α} ys :
    NonEmpty xs → NonEmpty (xs ++ ys) := by
  coaes
```

CoAes finds this proof in four steps:

- A built-in rule introduces the hypothesis `h : NonEmpty xs`. By default,
  CoAes's rule set contains a number of straightforward rules for handling the
...
If you want to see how CoAes proves your goal (or why it doesn't prove your
goal, or why it takes too long to prove your goal), you can enable tracing:

``` lean
set_option trace.coaes true
```

This makes CoAes print out the steps it takes while searching for a proof. You
can also look at the search tree CoAes constructed by enabling the
`trace.coaes.tree` option. For more tracing options, type `set_option
trace.coaes` and see what auto-completion suggests.

If, in the example above, you call `coaes?` instead, then CoAes prints a proof
...
With a bit of post-processing, you can use this script instead of the CoAes
call. This way you avoid the performance penalty of making CoAes search for a
...
The `@[coaes]` attribute on `nonEmpty_append₁` adds this lemma as an **unsafe**
...
goal, CoAes may still try other rules to solve the same goal. This makes sense
...
give us goals with priority 50%, 25%, etc. CoAes always considers the
highest-priority unsolved goal first, so it prefers proof attempts involving few
and high-probability rules. Additionally, when CoAes has a choice between
...
After adding `nonEmpty_append`, CoAes can prove some consequences of this
lemma:
...
theorem nil_not_nonEmpty (xs : MyList α) : xs = nil → ¬ NonEmpty xs := by
  coaes (add 10% cases MyList)
```

Here we use an **`add`** clause to add a rule which is only used in this
specific CoAes call. The rule is an unsafe `cases` rule for `MyList`. (As you
...
probability, to make sure that CoAes applies other rules if possible.

Here are some examples where CoAes's normalisation phase is particularly useful:
...
set, CoAes can prove theorems about this function more or less by itself (though
...
Many more examples can be found in the `CoAesTest` folder of this repository. In
particular, the file `CoAesTest/List.lean` contains an CoAes-ified port of 200
basic list lemmas from the Lean 3 version of mathlib, and the file
`CoAesTest/SeqCalcProver.lean` shows how CoAes can help with the formalisation
of a simple sequent calculus prover.

## Reference

This section contains a systematic and fairly comprehensive account of how CoAes
operates.
...
- **Safe rules** (keyword `safe`) are applied after normalisation but before any
  ...
  what is provable depends on the entire CoAes rule set.
...
CoAes's central data structure is a search tree. This tree alternates between
two kinds of nodes:
...
the goal which CoAes is supposed to solve. Then we perform the following steps
in a loop, stopping if (a) the root goal has been proved; (b) the root goal
becomes unprovable; or (c) one of CoAes's rule limits has been reached. (There
...
- If the goal of `G` has not been normalised yet, normalise it. That means we
  ...
  - Run `simp` on all hypotheses and the target, using the global simp set (i.e.
    lemmas tagged `@[simp]`) plus CoAes's `simp` rules.
...
A **rule builder** is a metaprogram that turns an expression into an CoAes rule.
When you tag a declaration with the `@[coaes]` attribute, the builder is applied
...
- **`forward`**: when applied to a term of type `A₁ → ... Aₙ → B`, generates a
  ...
  CoAes applies a forward rule with explicit immediate names, it only matches
  the corresponding arguments to hypotheses. (Here, `even_or_odd` has only one
  argument, so there is no difference.)

  When no immediate names are given, CoAes considers every argument immediate,
...
- **`tactic`**: takes a tactic and directly turns it into a rule. When this
  ...
  use `(by ...)` in an `add` clause, CoAes automatically uses the tactic
  builder, unless you specify a different builder.

  When this builder is used in the `@[coaes]` attribute, the declaration tagged
  with the attribute must have type `TacticM Unit`, `CoAes.SingleRuleTac` or
  `CoAes.RuleTac`. The latter are CoAes data types which associate a tactic with
...
``` lean
declare_coaes_rule_sets [r₁, ..., rₙ] (default := <bool>)
```
...
CoAes will consider these rules the same and arbitrarily pick one.

Out of the box, CoAes uses the default rule sets `builtin` and `default`. The
`builtin` set contains built-in rules for handling various constructions (see
below). The `default` set contains rules which were added by CoAes users without
specifying a rule set.

### The `@[coaes]` Attribute

Declarations can be added to rule sets by annotating them with the `@[coaes]`
attribute. As with other attributes, you can use `@[local coaes]` to add a rule
only within the current section or namespace and `@[scoped coaes]` to add a rule
...
``` lean
@[coaes <phase>? <priority>? <builder>? <builder_option>* <rule_sets>?]
```
...
``` lean
@[coaes unsafe [constructors 75%, cases 90%]]
inductive T ...

@[coaes apply [safe (rule_sets := [A]), 70% (rule_sets := [B])]]
def foo ...

@[coaes [80% apply, safe 5 forward (immediate := x)]]
def bar (x : T) ...
```
...
In general, the grammar for the `@[coaes]` attribute is

``` lean
attr      ::= @[coaes <rule_expr>]
            | @[coaes [<rule_expr,+>]]
...
```lean
attribute [coaes norm unfold] List.all -- List.all is from Init
```

You can also use the `add_coaes_rules` command:

``` lean
add_coaes_rules safe [(by linarith), Nat.add_comm 0]
```

As you can see, this command can be used to add tactics and composite terms as
well. Use `local add_coaes_rules` and `scoped add_coaes_rules` to obtain the
equivalent of `@[local coaes]` and `@[scoped coaes]`.

### Erasing Rules

There are two ways to erase rules. Usually it suffices to remove the `@[coaes]`
attribute:
...
If you want to remove only certain rules, you can use the `erase_coaes_rules`
command:

``` lean
erase_coaes_rules [safe apply foo, bar (rule_sets := [A])]
```
...
``` lean
erase_coaes_rules [<rule_expr,+>]
```

i.e. rules are specified in the same way as for the `@[coaes]` attribute.
...
In its most basic form, you can call the CoAes tactic just by writing
...
`default` rule set, containing rules tagged with the `@[coaes]` attribute
without mentioning a specific rule set, and the `builtin` rule set, containing
rules built into CoAes. However, other rule sets can also be enabled by default;
see the `declare_coaes_rule_sets` command.

The tactic's behaviour can also be customised with various options. A more
involved CoAes call might look like this:
...
#### Adding Rules to an CoAes Call

Rules can be added to an CoAes call with an `add` clause. This won't affect any
...
i.e. rules can be specified in the same way as for the `@[coaes]` attribute.
As with the `erase_coaes_rules` command, each rule must specify the name of
...
#### Erasing Rules From an CoAes Call

Rules can be removed from an CoAes call with an `erase` clause. Again, this
affects only the current CoAes call and not the declared rule sets. The syntax
...
and it works exactly like the `erase_coaes_rules` command. To erase all rules
...
By default, CoAes uses the `default` and `builtin` rule sets, as well as rule
...
The term is an arbitrary Lean expression of type `CoAes.Options`; see there for
...
so for now I won't document them in detail. See `CoAes/BuiltinRules.lean` and
`CoAes/BuiltinRules/*.lean`

### Proof Scripts

By calling `coaes?` instead of `coaes`, you can instruct CoAes to generate a
tactic script which proves the goal (if CoAes succeeds). The script is printed
as a `Try this:` suggestion, similar to `simp?`.

The scripts generated by CoAes are currently a bit idiosyncratic. For example,
they may contain the `coaes_cases` tactic, which is a slight variation of the
standard `cases`. Additionally, CoAes occasionally generates buggy scripts which
...
To see how CoAes proves a goal -- or why it doesn't prove a goal, or why it's
...
- `trace.coaes`: print a step-by-step log of which goals CoAes tried to
  solve, which rules it tried to apply (successfully or unsuccessfully), etc.
- `trace.coaes.ruleSet`: print the rule set used for an CoAes call.
- `trace.coaes.proof`: if CoAes is successful, print the proof that was
  generated (as a Lean term). You should be able to copy-and-paste this proof
  to replace CoAes.
  
### Profiling

To get an idea of where CoAes spends its time, use

``` lean
set_option trace.coaes.stats true
```

CoAes then prints some statistics about this particular CoAes run.

To get statistics for multiple CoAes invocations, activate the option
`coaes.collectStats` for the relevant files (or for certain invocations) and run
the command `#coaes_stats` in a file which imports all relevant files. E.g. to
evaluate CoAes's performance in Mathlib, set the option `coaes.collectStats` in
...
by `trace.coaes` with information about how much time each step took. Note that
...
If you encounter behaviour that looks like an internal error in CoAes, it may
help to set the option `coaes.check.all` (or the more fine-grained
`coaes.check.*` options). This makes CoAes check various invariants while the
...
Rules which create metavariables must be handled specially by CoAes. For
example, suppose we register transitivity of `<` as an CoAes rule. Then we may
...
proven). CoAes could also decide to work on the second subgoal first, in which
case the situation is symmetric.

Due to this dependency, CoAes in effect treats the instantiations of the second
subgoal as *additional goals*. Thus, when we apply the theorem `∀ n, n < n + 1`,
which closes the first goal, CoAes realises that because this theorem was
...
goals, which CoAes may visit in any order, CoAes may spend a lot of time copying
...
global rule set and added to individual CoAes calls on an ad-hoc basis.
...
For more details on the handling of metavariables, see the [CoAes
