...
set_option coaes.check.script true

-- We use this constant to 'prove' theorems which CoAes can't solve. We don't
...
@[coaes norm forward (immediate := [h])]
...
attribute [-coaes] CoAes.BuiltinRules.ext
...
attribute [coaes safe constructors] List.Mem
attribute [coaes safe cases (cases_patterns := [List.Mem _ [], List.Mem _ (_ :: _)])] List.Mem
...
theorem X.eq_or_ne_mem_of_mem {a b : α} {l : List α} : a ∈ b :: l → a = b ∨ (a ≠ b ∧ a ∈ l) := by
  open Classical in
  coaes (add safe [decidable.list.eq_or_ne_mem_of_mem])
...
theorem mem_map_of_injective {f : α → β} (H : Injective f) {a : α} {l : List α} :
  f a ∈ map f l ↔ a ∈ l := by
  set_option coaes.check.script false in -- TODO pp.analyze bug?
  coaes
...
theorem X.length_pos_of_mem {a : α} : ∀ {l : List α}, a ∈ l → 0 < length l := by
  coaes (add 1% cases List) (simp_config := { arith := true })
...
theorem X.length_pos_iff_exists_mem {l : List α} : 0 < length l ↔ ∃ a, a ∈ l := by
  coaes (add unsafe [length_pos_of_mem, exists_mem_of_length_pos])
...
theorem length_pos_iff_ne_nil {l : List α} : 0 < length l ↔ l ≠ [] := by
  coaes (add unsafe [ne_nil_of_length_pos, length_pos_of_ne_nil])
...
theorem exists_of_length_succ {n} :
  ...
  intro l; induction l <;> coaes (simp_config := { arith := true })
...
@[simp] theorem X.cons_subset {a : α} {l m : List α} :
  a::l ⊆ m ↔ a ∈ m ∧ l ⊆ m := by
  coaes (add norm simp [HasSubset.Subset, List.Subset])
...
theorem X.eq_nil_iff_forall_not_mem {l : List α} : l = [] ↔ ∀ a, a ∉ l :=
  ADMIT
  -- used to be by `coaes (add 1% cases List)` until simp changes around nightly
  -- 2024-03-11
...
theorem map_subset_iff {l₁ l₂ : List α} (f : α → β) (h : Injective f) :
  ...
  · set_option coaes.check.script false in
    coaes -- TODO timeout in script generation
...
theorem X.cons_eq_append_iff {a b c : List α} {x : α} :
  ...
  coaes (add norm simp [append_eq_cons_iff, eq_comm])
...
theorem X.append_inj :
  ...
  | []     , []     , t₁, t₂, h, _  => by coaes
  | (a::s₁), []     , t₁, t₂, _, hl => by coaes
  | []     , (b::s₂), t₁, t₂, _, hl => by coaes
  | (a::s₁), (b::s₂), t₁, t₂, h, hl => by
...
theorem X.eq_replicate_of_mem {a : α} {l : List α} : (∀ b, b ∈ l → b = a) → l = replicate l.length a := by
  induction l <;> coaes (config := { useSimpAll := false })
...
theorem X.eq_replicate {a : α} {n} {l : List α} : l = replicate n a ↔ length l = n ∧ ∀ b, b ∈ l → b = a := by
  coaes (add norm simp eq_replicate')
...
theorem flatMap_assoc' {α β γ : Type u} (l : List α) (f : α → List β) (g : β → List γ) :
  ...
  --   induction xs <;> coaes
  -- induction l <;> coaes (add norm [simp [flatMap_append], unfold [flatMap]])
...
@[simp] theorem X.reverse_cons (a : α) (l : List α) : reverse (a::l) = reverse l ++ [a] :=
  ...
  --   intro l₁; induction l₁ <;> coaes (add norm unfold reverseAux)
  -- coaes (add norm unfold reverse)
...
@[simp] theorem X.reverseAux_eq (l₁ l₂ : List α) : reverseAux l₁ l₂ = reverse l₁ ++ l₂ := by
  induction l₁ generalizing l₂ <;> coaes
...
-- TODO: after nightly-2024-08-27, `coaes` can not prove this anymore!
...
@[simp] theorem reverse_inj' {l₁ l₂ : List α} : reverse l₁ = reverse l₂ ↔ l₁ = l₂ := by
  coaes (add safe forward reverse_injective)
...
@[simp] theorem reverse_eq_nil {l : List α} : reverse l = [] ↔ l = [] := by
  coaes (add norm simp reverse_eq_iff)
...
theorem map_reverse_core (f : α → β) (l₁ l₂ : List α) :
  ...
  coaes (add norm simp map_reverse')
...
@[simp] theorem length_init : ∀ (l : List α), length (init l) = length l - 1
  ...
  | (_ :: y :: zs) => by
    ...
    coaes (add norm simp [init, Nat.add_sub_cancel])
...
theorem last_mem : ∀ {l : List α} (h : l ≠ []), last l h ∈ l := by
  intro l; induction l <;> coaes (add norm simp last, 1% cases List)
