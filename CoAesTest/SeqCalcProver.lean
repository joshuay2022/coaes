...
set_option coaes.check.script.steps true

--- Decidable

attribute [coaes unsafe [50% constructors, 50% cases]] Decidable
attribute [coaes safe apply] instDecidableAnd
...
theorem Mem.split [DecidableEq α] {xs : List α} {v : α} (h : v ∈ xs)
  ...
  case cons x xs ih =>
    ...
    cases dec
    case isFalse no =>
      let ⟨l, r, eq⟩ : ∃ l r, xs = l ++ v :: r := by coaes
      rw [eq]
      exact ⟨x :: l, r, rfl⟩
    case isTrue yes =>
      rw [yes]
...
-- TODO: the trace.coaes.proof does not work here
theorem in_self (xs : List α) : All (· ∈ xs) xs := by
  induction xs <;> coaes (add unsafe apply weaken)
...
@[coaes safe cases (cases_patterns := [Any _ []]),
  coaes unsafe [50% constructors, 50% cases (cases_patterns := [Any _ (_ :: _ )])]]
...
@[coaes unsafe [50% constructors, 25% cases (cases_patterns := [Perm (_ :: _) (_ :: _ )])]]
...
theorem all {i : Φ → Prop} {Γ Γ' Δ : List (Form Φ)} (perm : Γ' ↭ Γ) (h : SC i Γ Δ)
  : SC i Γ' Δ := by coaes (add unsafe apply Perm.all)

theorem any {i : Φ → Prop} {Γ Δ Δ' : List (Form Φ)} (perm : Δ ↭ Δ') (h : SC i Γ Δ)
  : SC i Γ Δ' := by coaes (add unsafe apply Perm.any)
...
attribute [coaes safe apply] Proof.basic Proof.fls_l
attribute [coaes unsafe 50% apply] Proof.imp_l Proof.imp_r
attribute [coaes unsafe 20% apply] Proof.per_l Proof.per_r
...
theorem weaken (Γ Δ : List (Form Φ)) (prf : Proof Γ Δ) (δ : Form Φ)
  ...
  induction prf with
  | imp_r Γ Δ φ ψ =>
    have ih' : Proof (φ :: Γ) (ψ :: δ :: Δ) := by coaes
    coaes
  | _ => coaes (config := { maxRuleApplications := 250 })
...
theorem sound (i : Φ → Prop) [DecidablePred i] (prf : Proof Γ Δ) : SC i Γ Δ := by
  ...
  | imp_r Γ Δ φ ψ _ ih =>
    ...
    coaes
  | _ => coaes (add unsafe apply [Perm.all, Perm.any])
...
theorem Cal_Proof [DecidableEq Φ]
  ...
  | [], [] =>
    ...
    have p' : Proof' (ll ++ a :: lr) (a :: rl ++ rr) [] [] := by
      coaes (add unsafe apply [Perm.map, Perm.shift])
    coaes (add unsafe apply [Perm.map, Perm.shift, Perm.sym])
  | ⊥ :: Γ, [] =>
    coaes
  | Γ, ⊥ :: Δ =>
    simp at h
  ...
  | ♩n :: Γ, [] =>
    ...
    coaes (add unsafe apply [Perm.shift, Perm.sym])
  | Γ, ♩n :: Δ =>
    simp at h
    have ih : Proof' l (n :: r) Γ Δ := Cal_Proof l (n :: r) Γ Δ h
    coaes (add unsafe apply [Perm.shift])
  | φ ⇒ ψ :: Γ, [] =>
    simp at h
