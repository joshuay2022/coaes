...
set_option coaes.dev.statefulForward false

open CoAes Lean Lean.Meta Lean.Elab.Tactic
...
/-! # Tests for CoAes's forward rules -/

example (a : α) (b : β) (r₁ : (a : α) → (b : β) → γ₁ ∧ γ₂)
    (r₂ : (a : α) → δ₁ ∧ δ₂) : γ₁ ∧ γ₂ ∧ δ₁ ∧ δ₂ := by
  coaes (add safe [forward r₁, forward (immediate := [a]) r₂])
...
example (a : α) (b : β) (r₁ : (a : α) → (b : β) → γ)
    (r₂ : (a : α) → δ) : γ ∧ δ := by
  coaes (add 1% [destruct r₁, destruct (immediate := [a]) r₂])
...
example {α : Prop} (a : α) (b : β) (r₁ : (a : α) → (b : β) → γ)
    (r₂ : (a : α) → δ) : γ ∧ δ := by
  coaes (add safe [destruct r₁, destruct (immediate := [a]) r₂])
    (config := { enableSimp := false, terminal := true })

example (a : α) (b : β) (r₁ : (a : α) → (b : β) → γ₁ ∧ γ₂)
    (r₂ : (a : α) → δ₁ ∧ δ₂) : γ₁ ∧ γ₂ ∧ δ₁ ∧ δ₂ := by
  coaes (add safe [forward r₁], 90% destruct r₂)
