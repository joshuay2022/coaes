...
-- This test checks whether the output of trace.coaes.proof is
-- copy-and-pastable. When the test breaks because CoAes's output has changed,
...
theorem All.split_cons (P : α → Prop) (x : α) (xs : List α) (h : All P (x :: xs))
  : P x ∧ All P xs := by
  set_option trace.coaes.proof true in
  coaes
