...
theorem Perm.any {xs ys : List α} (perm : Perm xs ys) (P : α → Prop)
  ...
  induction perm <;> coaes (add safe [constructors Any, cases Any])

/--
error: tactic 'coaes' failed, maximum number of rule applications (100) reached. Set the 'maxRuleApplications' option to increase the limit.
-/
#guard_msgs in
theorem error (P : Nat → Prop) (Δ : List Nat) : Any P Δ := by
  coaes (add 50% [constructors Perm, constructors Any, Perm.any])
...
theorem fine (P : α → Prop) (Δ : List α) : Any P Δ := by
  coaes (add unsafe [50% constructors Perm, 50% constructors Any, apply 50% Perm.any])
