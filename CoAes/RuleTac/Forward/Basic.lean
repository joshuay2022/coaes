...
def forwardImplDetailHypPrefix := `__coaes.fwd
...
def matchForwardImplDetailHypName (n : Name) : Option (Nat × Name) :=
  ...
  | `__coaes :: `fwd :: .num .anonymous depth :: components =>
...
def _root_.CoAes.clearForwardImplDetailHyps (goal : MVarId) : MetaM MVarId :=
  goal.withContext do
