...
public meta import CoAes.Saturate
public meta import CoAes.Frontend.Extension
public meta import CoAes.Builder.Forward
...
def _root_.CoAes.ElabM.runForwardElab (goal : MVarId) (x : ElabM α) :
    TermElabM α :=
  x |>.run { parsePriorities := true, goal }

def mkForwardOptions (maxDepth? : Option Nat) (traceScript : Bool) :
    CoreM Options' :=
  ({ traceScript } : CoAes.Options).toOptions' maxDepth?
