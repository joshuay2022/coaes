...
public import CoAes.Frontend.Extension.Init
...
def extensionDescr (rsName : RuleSetName) :
  ...
  -- The extension name is prefixed with `coaes` so that it cannot collide
  -- with Aesop's rule set extensions (`default`, `builtin`, ...) when both
  -- libraries are loaded (e.g. CoAes together with Mathlib).
  name := `coaes ++ rsName
...
def declareRuleSetUnchecked (rsName : RuleSetName) (isDefault : Bool) :
  ...
  let simpExtName := .mkStr1 s!"coaes_{rsName}"
  discard $ registerSimpAttr simpExtName (ref := simpExtName)
    s!"simp theorems in the CoAes rule set '{rsName}'"
  let simprocExtName := .mkStr1 s!"coaes_{rsName}_proc"
  discard $ Simp.registerSimprocAttr simprocExtName (name := simprocExtName)
    (ref? := none) s!"simprocs in the CoAes rule set '{rsName}'"
...
def addGlobalRule (rsName : RuleSetName) (r : GlobalRuleSetMember)
  ...
  if checkNotExists then
    ...
    if rs.contains r.name then
      throwError "coaes: rule '{r.name.name}' is already registered in rule set '{rsName}'"
  match r with
