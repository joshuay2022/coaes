...
public import CoAes.ElabM
public import CoAes.Percent
public import CoAes.Rule.Name
public import CoAes.Builder.Cases
public import CoAes.Builder.Default
public import CoAes.Builder.Forward
public import CoAes.Builder.Unfold
public import CoAes.RuleSet.Filter
...
declare_syntax_cat CoAes.priority

syntax num "%" : CoAes.priority
syntax "-"? num : CoAes.priority
...
declare_syntax_cat CoAes.phase (behavior := symbol)

syntax "safe" : CoAes.phase
syntax "norm" : CoAes.phase
syntax "unsafe" : CoAes.phase
...
declare_syntax_cat CoAes.builder_name (behavior := symbol)

syntax "apply" : CoAes.builder_name
syntax "simp" : CoAes.builder_name
syntax "unfold" : CoAes.builder_name
syntax "tactic" : CoAes.builder_name
syntax "constructors" : CoAes.builder_name
syntax "forward" : CoAes.builder_name
syntax "destruct" : CoAes.builder_name
syntax "cases" : CoAes.builder_name
syntax "default" : CoAes.builder_name
...
declare_syntax_cat CoAes.indexing_mode (behavior := symbol)

syntax "target " term : CoAes.indexing_mode
syntax "hyp " term : CoAes.indexing_mode
syntax "unindexed " : CoAes.indexing_mode
...
declare_syntax_cat CoAes.builder_option

syntax " (" &"immediate" " := " "[" ident,+,? "]" ")" : CoAes.builder_option
syntax " (" &"index" " := " "[" CoAes.indexing_mode,+,? "]" ")" : CoAes.builder_option
syntax " (" &"pattern" " := " term ")" : CoAes.builder_option
syntax " (" &"cases_patterns" " := " "[" term,+,? "]" ")" : CoAes.builder_option
syntax " (" &"transparency" " := " transparency ")" : CoAes.builder_option
syntax " (" &"transparency!" " := " transparency ")" : CoAes.builder_option
...
def «elab» (stx : TSyntax `CoAes.builder_option) : ElabM BuilderOption :=
  withRef stx do
    match stx with
    | `(builder_option| (immediate := [$ns:ident,*])) =>
      return immediate $ (ns : Array Syntax).map (·.getId)
    | `(builder_option| (index := [$imodes:CoAes.indexing_mode,*])) =>
...
declare_syntax_cat CoAes.feature (behavior := symbol)

-- NOTE: This grammar has overlapping rules `ident`, `CoAes.phase` and
-- `CoAes.builder_name`, which can all consist of a single ident. For ambiguous
-- parses, a `choice` node with two children is created; one being an
-- `CoAes.phase` or `CoAes.builder_name` node and the other being a `featIdent`
-- node. When we process these `choice` nodes, we select the non-`ident` one.

syntax CoAes.phase : CoAes.feature
syntax CoAes.priority : CoAes.feature
syntax CoAes.builder_name : CoAes.feature
syntax CoAes.builder_option : CoAes.feature
syntax ruleSetsFeature : CoAes.feature
syntax (name := featIdent) ident : CoAes.feature
syntax "(" term ")" : CoAes.feature
...
partial def «elab» (stx : Syntax) : ElabM Feature :=
  withRef stx do
    match stx with
    | `(feature| $p:CoAes.priority) => priority <$> Priority.elab p
    | `(feature| $p:CoAes.phase) => phase <$> PhaseName.elab p
    | `(feature| $b:CoAes.builder_name) => builder <$> DBuilderName.elab b
    | `(feature| $o:CoAes.builder_option) => builderOption <$> BuilderOption.elab o
...
declare_syntax_cat CoAes.rule_expr (behavior := symbol)

syntax CoAes.feature : CoAes.rule_expr
syntax CoAes.feature ppSpace CoAes.rule_expr : CoAes.rule_expr
syntax CoAes.feature " [" CoAes.rule_expr,+,? "]" : CoAes.rule_expr
...
partial def «elab» (stx : Syntax) : ElabM RuleExpr :=
  withRef stx do
    match stx with
    | `(rule_expr| $f:CoAes.feature $e:CoAes.rule_expr) => do
      return node (← Feature.elab f) #[← «elab» e]
    | `(rule_expr| $f:CoAes.feature [ $es:CoAes.rule_expr,* ]) => do
      return node (← Feature.elab f) (← (es : Array Syntax).mapM «elab»)
    | `(rule_expr| $f:CoAes.feature) => do
