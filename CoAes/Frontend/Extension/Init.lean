...
/--
An environment extension containing an CoAes rule set. Each rule set has its
own extension.
-/
...
/--
Structure containing information about all declared CoAes rule sets.
-/
structure DeclaredRuleSets where
  ...
  /--
  The set of CoAes rule sets that are enabled by default.
  -/
