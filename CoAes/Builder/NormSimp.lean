...
def PhaseSpec.getSimpPrio [Monad m] [MonadError m] : PhaseSpec → m Nat
  | .norm info =>
    if info.penalty ≥ 0 then
      ...
      throwError "coaes: simp rules must be given a non-negative integer priority"
  | _ => throwError "coaes: simp builder can only construct 'norm' rules"

namespace RuleBuilder

def simpCore (decl : Name) (phase : PhaseSpec) : MetaM LocalRuleSetMember :=
  withExceptionTransform (λ msg => m!"coaes: simp builder: exception while trying to add {decl} as a simp theorem:{indentD msg}") do
