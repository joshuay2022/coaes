...
public import CoAes.Builder.Basic
  ...
  | some i =>
    withoutModifyingState do
      ...
      | some arg =>
        ...
        return .hyps keys
      | none => throwError
        "coaes: internal error: immediate arg for forward rule is out of range"
...
def forwardCore₂ (t : ElabRuleTerm) (immediate? : Option (Array Name))
  ...
  coaes_trace[forward] "rule type:{indentExpr $ ← inferType expr}"
  withConstCoAesTraceNode .forward (return m!"slot clusters") do
    coaes_trace[forward] do
      for h : i in [:info.slotClusters.size] do
        let cluster := info.slotClusters[i]
        withConstCoAesTraceNode .forward (return m!"cluster {i}") do
          for s in cluster do
            coaes_trace![forward] "slot {s.index} (premise {s.premiseIndex}, deps {s.deps.toArray.qsortOrd}, common {s.common.toArray.qsortOrd}, forward deps {s.forwardDeps.qsortOrd})"
  coaes_trace[forward] "conclusion deps: {info.conclusionDeps}"
  let prio :=
    match phase with
...
def forwardCore (t : ElabRuleTerm) (immediate? : Option (Array Name))
  ...
  coaes_trace[debug] "decl type: {type}"
  let immediate ← getImmediatePremises type pat? immediate?
  coaes_trace[debug] "immediate premises: {immediate}"
  let imode ← getForwardIndexingMode type immediate
  coaes_trace[debug] "imode: {imode}"
...
def forward (isDestruct : Bool) : RuleBuilder := λ input => do
  withConstCoAesTraceNode .debug (return "forward builder") do
