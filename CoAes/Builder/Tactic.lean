  ...
  | .inl decl =>
    ...
    let tac ←
      if ← isDefEq (mkApp (mkConst ``TacticM) (mkConst ``Unit)) type then
        ...
        throwError "coaes: tactic builder: expected {decl} to be a tactic, i.e. to have one of these types:\n  TacticM Unit\n  SimpleRuleTac\n  RuleTac\n  TacGen\nHowever, it has type{indentExpr type}"
...
def tactic : RuleBuilder := λ input => do
  ...
  let t ←
    if input.term.raw.isIdent then
      ...
      throwError "coaes: tactic builder: expected '{input.term}' to be a tactic"
