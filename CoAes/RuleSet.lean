...
public import CoAes.Index
public import CoAes.Index.Forward
public import CoAes.RuleSet.Filter
public import CoAes.RuleSet.Member
public import CoAes.Tree.Data.ForwardRuleMatches
...
/--
The CoAes-specific parts of an CoAes rule set. A `BaseRuleSet` stores global
CoAes rules, e.g. safe and unsafe rules. It does not store simp theorems for
...
/--
A global CoAes rule set. When we tag a declaration with `@[coaes]`, it is stored
in one or more of these rule sets. Internally, a `GlobalRuleSet` is composed of
a `BaseRuleSet` (stored in an CoAes rule set extension) plus a set of simp
...
/--
The rule set used by an CoAes tactic call. A local rule set is produced by
...
def BaseRuleSet.trace (rs : BaseRuleSet) (traceOpt : TraceOption) :
  ...
  if ! (← traceOpt.isEnabled) then
    return
  withConstCoAesTraceNode traceOpt (return "Erased rules") do
    coaes_trace![traceOpt] "(Note: even if these rules appear in the sections below, they will not be applied by CoAes.)"
    let erased := rs.erased.fold (init := #[])
      λ ary r => ary.push r
    for r in erased.qsortOrd do
      coaes_trace![traceOpt] r
  withConstCoAesTraceNode traceOpt (return "Unsafe rules") do
    rs.unsafeRules.trace traceOpt
  withConstCoAesTraceNode traceOpt (return "Safe rules") do
    rs.safeRules.trace traceOpt
  withConstCoAesTraceNode traceOpt (return "Forward rules") do
    rs.forwardRules.trace traceOpt
  withConstCoAesTraceNode traceOpt (return "Normalisation rules") do
    rs.normRules.trace traceOpt
  withConstCoAesTraceNode traceOpt (return "Constants to unfold") do
...
def GlobalRuleSet.trace (rs : GlobalRuleSet) (traceOpt : TraceOption) :
  ...
  withConstCoAesTraceNode traceOpt (return "Normalisation simp theorems:") do
...
def LocalRuleSet.trace (rs : LocalRuleSet) (traceOpt : TraceOption) :
  ...
  if ! (← traceOpt.isEnabled) then
    return
  rs.toBaseRuleSet.trace traceOpt
  withConstCoAesTraceNode traceOpt (return "Simp sets used by normalisation simp:") do
    rs.simpTheoremsArray.map (printSimpSetName ·.fst) |>.qsortOrd.forM λ s => do
      coaes_trace![traceOpt] s
  withConstCoAesTraceNode traceOpt (return "Local normalisation simp theorems") do
...
private def fwdRulePredicate (opts : Lean.Options) (rs : LocalRuleSet)
  ...
  coaes.dev.statefulForward.get opts && include? r && ! rs.isErased r.name
...
private def rulePredicate (opts : Lean.Options) (rs : LocalRuleSet)
  ...
  if coaes.dev.statefulForward.get opts then
...
def forwardRulePatternSubstsInExpr (rs : LocalRuleSet) (e : Expr) :
  ...
  withConstCoAesTraceNode .forward (return m!"rule patterns in expr {e}:") do
    let ms ← rs.rulePatterns.get e
    let ms := postprocessPatSubstMap rs ms
    coaes_trace[forward] do
      for (r, inst) in ms do
        coaes_trace![forward] m!"{r}, {inst}"
    return ms

def forwardRulePatternSubstsInLocalDecl (rs : LocalRuleSet) (ldecl : LocalDecl) :
    BaseM (Array (ForwardRule × Substitution)) := do
  withConstCoAesTraceNode .forward (return m!"rule patterns in hyp {ldecl.userName}:") do
...
def unindexPredicate? (options : Options') : Option (RuleName → Bool) :=
  if options.destructProductsTransparency == .reducible then
    ...
    some λ n => n.name == `CoAes.BuiltinRules.destructProducts
