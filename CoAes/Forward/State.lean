...
public import CoAes.Forward.Match
...
def matchPremise? (premises : Array MVarId) (lmvarIds : Array LMVarId)
  ...
  let some slot := cs.slots[slot.toNat]?
    | throwError "coaes: internal error: matchPremise?: no slot with index {slot}"
  let premiseIdx := slot.premiseIndex.toNat
  let some slotPremise := premises[premiseIdx]?
    | throwError "coaes: internal error: matchPremise?: slot with premise index {premiseIdx}, but only {premises.size} premises"
  let premiseType ← slotPremise.getType
  let hypType ← hyp.getType
  withCoAesTraceNodeBefore .forward (return m!"match against premise {premiseIdx}: {hypType} ≟ {premiseType}") do
  withoutModifyingState do
    let isDefEq ←
      withConstCoAesTraceNode .forwardDebug (return m!"defeq check") do
      withReducible do
    ...
    if isDefEq then
      ...
      coaes_trace[forward] "substitution: {subst}"
      return subst
    else
      return none
where
  updateSubst (premises : Array MVarId) (var : PremiseIndex)
    ...
    withConstCoAesTraceNode .forwardDebug (return m!"update var {var}") do
      let some varMVarId := premises[var.toNat]?
        | throwError "coaes: internal error: matchPremise?: dependency with index {var}, but only {premises.size} premises"
      let mvar := .mvar varMVarId
      let assignment ← instantiateMVars mvar
      if assignment == mvar then
        throwError "coaes: internal error: matchPremise?: while matching hyp {hyp.name}: no assignment for variable {var}"
      if ← hasAssignableMVar assignment then
        throwError "coaes: internal error: matchPremise?: assignment has mvar:{indentExpr assignment}"
...
mutual
  ...
  partial def addMatch (cs : ClusterState) (m : Match) : AddM ClusterState := do
    ...
    if slotIdx.toNat == cs.slots.size - 1 then
      if cs.completeMatches.contains m then
        coaes_trace[forward] "complete match {m} already present"
        return cs
      else
  ...
  partial def addHyp (cs : ClusterState) (slot : Slot) (h : Hyp) :
    ...
    withConstCoAesTraceNode .forward (return m!"add hyp or pattern inst for slot {slot.index} with substitution {h.subst}") do
    if slot.index.toNat == 0 then
      ...
      if ! changed then
        coaes_trace[forward] "hyp already present"
        return cs
  ...
  partial def addRawHypCore (h : RawHyp) (slot : Slot) (cs : ClusterState) :
    ...
    match h with
    | .fvarId fvarId =>
      withConstCoAesTraceNode .forwardDebug (return m!"add hyp {Expr.fvar fvarId} ({fvarId.name}) to slot {slot.index}") do
        let some subst ←
    ...
    | .patSubst subst =>
      withConstCoAesTraceNode .forwardDebug (return m!"add pattern subst {subst} to slot {slot.index}") do
  ...
  partial def addQueuedRawHyps (slot : Slot) (cs : ClusterState) :
      AddM ClusterState :=
    withConstCoAesTraceNode .forward (return m!"add queued hyps for slot {slot.index}") do
...
def addRawHyp (goal : MVarId) (h : RawHyp) (pi : PremiseIndex) (rs : RuleState) :
  ...
  withNewMCtxDepth do
    ...
    let some ruleExpr ←
      withConstCoAesTraceNode .forwardDebug (return m!"elab rule term") do
    ...
    if lmvars.size != rs.rule.numLevelParams then
      coaes_trace[forward] "failed to add hyp or pat inst: rule term{indentD $ toMessageData rs.rule.term}\ndoes not have expected number of level mvars {rs.rule.numLevelParams}"
      return (rs, #[])
    let ruleType ← instantiateMVars (← inferType ruleExpr)
    let (premises, _, _) ←
      withConstCoAesTraceNode .forwardDebug (return m!"open rule term") do
      withReducible do
    ...
    for i in [:clusterStates.size] do
      ...
      completeMatches ←
        withConstCoAesTraceNode .forwardDebug (return m!"construct new complete matches") do
...
private def addForwardRuleMatches (acc : Array ForwardRuleMatch)
  ...
  coaes_trace[forward] do
    for m in ruleMatches do
      coaes_trace![forward] "new complete match for {m.rule.name}:{indentD $ toMessageData m}"
  return ruleMatches
...
def addHypCore (ruleMatches : Array ForwardRuleMatch) (goal : MVarId)
  ...
  goal.withContext do
  withConstCoAesTraceNode .forward (return m!"add hyp {Expr.fvar h} ({h.name})") do
    let hTypeRPINF ← rpinf (← h.getType)
    if (← isProp hTypeRPINF.toExpr) && fs.hypTypes.contains hTypeRPINF then
      coaes_trace[forward] "a hyp with the same (propositional) type was already added"
    ...
    ms.foldlM (init := (fs, ruleMatches)) λ (fs, ruleMatches) (r, i) => do
      withConstCoAesTraceNode .forward (return m!"rule {r.name}, premise {i}") do
...
def addPatSubstCore (ruleMatches : Array ForwardRuleMatch) (goal : MVarId)
    ...
    BaseM (ForwardState × Array ForwardRuleMatch) :=
  goal.withContext do
  withConstCoAesTraceNode .forward (return m!"add pat inst {patSubst} to rule {r.name}") do
    let rs := fs.ruleStates.find? r.name |>.getD r.initialRuleState
    let some (_, patSlotPremiseIdx) := r.rulePatternInfo?
      | throwError "coaes: internal error: addPatSubstCore: rule {r.name} does not have a rule pattern"
