...
public import CoAes.Forward.PremiseIndex
public import CoAes.Forward.SlotIndex
public import CoAes.Rule
public import CoAes.Rule.Forward
public import CoAes.RuleTac.Descr
public import CoAes.RuleTac.ElabRuleTerm
public import CoAes.RuleTac.Forward.Basic
...
def getProof (goal : MVarId) (m : ForwardRuleMatch) : MetaM (Option Expr) :=
  ...
  withConstCoAesTraceNode .forward (return m!"proof construction for forward rule match") do
  goal.withContext do
  withNewMCtxDepth do
    coaes_trace[forward] "rule: {m.rule.name}"
    let e ← elabForwardRuleTerm goal m.rule.term
    let lmvars := collectLevelMVars {} e |>.result
    let eType ← instantiateMVars (← inferType e)
    coaes_trace[forward] "term: {e} : {eType}"
    ...
    coaes_trace[forward] "args:   {args.map λ | none => m!"_" | some e => m!"{e}"}"
    coaes_trace[forward] "levels: {levels.map λ | none => m!"_" | some l => m!"{l}"}"
    ...
    try
      synthAppInstances `coaes goal argMVars binderInfos
        (synthAssignedInstances := false) (allowSynthFailures := false)
    catch _ =>
      coaes_trace[forward] "instance synthesis failed"
      return none
    let result := (← abstractMVars $ mkAppN e argMVars).expr
    coaes_trace[forward] "result: {result}"
    return result
...
def apply (goal : MVarId) (m : ForwardRuleMatch) (skip? : Option (RPINF → Bool)) :
  ...
  withConstCoAesTraceNode .forward (return m!"apply complete match") do
  goal.withContext do
    ...
    if let some skip := skip? then
      let doSkip ← withConstCoAesTraceNode .forwardDebug (return m!"check whether hyp already exists") do
        let result := skip (← rpinf type)
        coaes_trace[forwardDebug] "already exists: {result}"
        pure result
      if doSkip then
        return none
