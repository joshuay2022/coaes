...
public import CoAes.Forward.State.ApplyGoalDiff
public import CoAes.RuleTac
public import CoAes.RuleTac.ElabRuleTerm
public import CoAes.Script.SpecificTactics
public import CoAes.Search.Expansion.Basic
public import CoAes.Search.Expansion.Simp
public import CoAes.Search.RuleSelection
public import CoAes.Search.SearchM
public import CoAes.Tree.State
public import Batteries.Lean.HashSet

public section

open Lean Lean.Meta CoAes.Script
...
structure State where
  ...
  /--
  Declarations that were already unfolded during the normalisation of this
  goal. Each unfold rule fires at most once per goal, so recursive definitions
  (which reappear in their own unfolding) do not cause an infinite unfold loop.
  -/
  unfoldedDecls : Std.HashSet Name := ∅
  deriving Inhabited
...
def withNormTraceNode (ruleName : DisplayRuleName)
  ...
  withCoAesTraceNode .steps fmt do
    let result? ← k
    if let some newGoal := result?.bind (·.newGoal?) then
      coaes_trace[steps] newGoal
    return result?
  where
...
def runNormRuleTac (rule : NormRule) (input : RuleTacInput) (fs : ForwardState)
  ...
  match result? with
  | .error e =>
    coaes_trace[steps] e.toMessageData
...
def mkNormSimpScriptStep
  ...
  let simpOnlyBuilder :=
    TacticBuilder.simpAllOrSimpAtStarOnly (simpAll := ctx.useHyps) preGoal
      ctx.configStx? usedTheorems
  -- When the default simp set is used, we prefer to record the simple tactic
  -- (`simp_all` / `simp at *`, listing only premises that are not part of the
  -- default simp set) and fall back to the exhaustive
  -- `simp only [...]`/`simp_all only [...]` form only if the simple tactic
  -- does not reproduce the recorded proof state.
  let tacticBuilders :=
    if (← read).options.useDefaultSimpSet then
      #[simpBuilder, simpOnlyBuilder]
    else
      #[simpOnlyBuilder]
  return {
      ...
      if ctx.useHyps then
        let (ctx, simprocs) ←
          addLocalRules localRules ctx.toContext ctx.simprocs
            (isSimpAll := true)
        CoAes.simpAll goal ctx simprocs
      else
        let (ctx, simprocs) ←
          addLocalRules localRules ctx.toContext ctx.simprocs
            (isSimpAll := false)
        CoAes.simpGoalWithAllHypotheses goal ctx simprocs
    ...
    let result ←
      ...
      | .solved .. =>
        ...
        if anyMVarDropped then
          coaes_trace[steps] "Normalisation simp solved the goal but dropped some metavariables. Skipping normalisation simp."
          show MetaM _ from restoreState preState
          pure .unchanged
        else
          pure result
      | .unchanged .. =>
        coaes_trace[steps] "norm simp left the goal unchanged"
        pure result
      | .simplified .. =>
        pure result
...
def normUnfoldCore (goal : MVarId) : NormM (Option NormRuleResult) := do
  ...
  -- Each declaration is unfolded at most once per goal. This makes it safe to
  -- register recursive definitions (e.g. via `coaes (defs := [...])`) as
  -- unfold rules: they reappear in their own unfolding, which would otherwise
  -- make the normalisation loop unfold them forever.
  let alreadyUnfolded := (← getThe NormM.State).unfoldedDecls
  let unfold? := λ n =>
    if alreadyUnfolded.contains n then none else unfoldRules.find? n
  let (result, steps) ← unfoldManyStarS goal unfold? |>.run
  match result with
  | none =>
    coaes_trace[steps] "nothing to unfold"
    return none
  | some (newGoal, usedDecls) =>
    modifyThe NormM.State λ s =>
      { s with unfoldedDecls := usedDecls.foldl (·.insert ·) s.unfoldedDecls }
    applyDiffToForwardState (← diffGoals goal newGoal)
    ...
    | .unchanged =>
      if h : step.val + 1 < steps.size then
        ...
        if anySuccess then
          ...
          return .unchanged
  throwError "coaes: exceeded maximum number of normalisation iterations ({maxIterations}). This means normalisation probably got stuck in an infinite loop."
...
def unfold : NormStep
  | goal, _, _ => do
    if ! (← readThe NormM.Context).options.enableUnfold then
      coaes_trace[steps] "norm unfold is disabled (options := \{ ..., enableUnfold := false })"
      return .unchanged
...
def simp (mvars : Std.HashSet MVarId) : NormStep
  | goal, _, _ => do
    if ! (← readThe NormM.Context).normSimpContext.enabled then
      coaes_trace[steps] "norm simp is disabled (simp_options := \{ ..., enabled := false })"
      return .unchanged
...
partial def normalizeGoalMVar (goal : MVarId)
  ...
  -- `unfold` runs first: unfolding registered definitions has the highest
  -- priority of all rules.
  let mut normSteps := #[
    NormStep.unfold,
...
def normalizeGoalIfNecessary (gref : GoalRef) [CoAes.Queue Q] :
    SearchM Q Bool := do
  let g ← gref.get
