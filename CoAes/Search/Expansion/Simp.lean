...
then CoAes `simp` would diverge from default `simp`, so we would have to adjust
the `coaes?` output as well. Instead, we add `let` hypotheses explicitly. This
...
def simpGoalWithAllHypotheses (mvarId : MVarId) (ctx : Simp.Context)
  ...
  mvarId.withContext do
    ...
    CoAes.simpGoal mvarId ctx simprocs discharge? simplifyTarget fvarIdsToSimp
      stats
