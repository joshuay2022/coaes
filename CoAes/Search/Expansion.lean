...
public import CoAes.Search.Expansion.Norm
public import CoAes.Tree.AddRapp
...
def runRegularRuleTac (goal : Goal) (tac : RuleTac) (ruleName : RuleName)
  ...
  let some (postNormGoal, postNormState) := goal.postNormGoalAndMetaState? | throwError
    "coaes: internal error during expansion: expected goal {goal.id} to be normalised (but not proven by normalisation)."
  let input := {
    goal := postNormGoal
    mvars := goal.mvars
...
def withRuleTraceNode (ruleName : RuleName)
  ...
  withCoAesTraceNode .steps fmt k
  where
...
def runRegularRuleCore (parentRef : GoalRef) (rule : RegularRule)
  ...
  | .error exc =>
    coaes_trace[steps] exc.toMessageData
    return none
  | .ok { applications := #[], .. } =>
    coaes_trace[steps] "Rule returned no rule applications"
    return none
  | .ok output =>
    return some output
  ...
  profilingRule (.ruleName matchResult.rule.name) (·.isSuccessfulOrPostponed) do
    ...
    withRuleTraceNode rule.name (·.toEmoji) "" do
      ...
      if rapps.size != 1 then
        coaes_trace[steps] "Safe rule did not produce exactly one rule application"
      ...
      if anyParentMVarAssigned then
        coaes_trace[steps] "Safe rule assigned metavariables, so we postpone it"
...
def expandGoal (gref : GoalRef) : SearchM Q RuleResult := do
  let provedByNorm ←
    withCoAesTraceNode .steps fmtNorm (normalizeGoalIfNecessary gref)
  coaes_trace[steps] do
    unless provedByNorm do
      let (goal, metaState) ←
        (← gref.get).currentGoalAndMetaState (← getRootMetaState)
      metaState.runMetaM' do
        coaes_trace![steps] "Goal after normalisation:{indentD goal}"
  if provedByNorm then
    return .proved #[]
  let safeResult ←
    withCoAesTraceNode .steps fmtSafe (runFirstSafeRule gref)
  ...
  where
    doUnsafe (postponedSafeRules : Array PostponedSafeRule) :
        SearchM Q RuleResult := do
      withCoAesTraceNode .steps fmtUnsafe do
