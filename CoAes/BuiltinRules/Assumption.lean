...
@[coaes safe -50 (rule_sets := [builtin])]
meta def assumption : RuleTac := λ input => do
  let goal := input.goal
  let md := input.options.assumptionTransparency
  goal.withContext do
    goal.checkNotAssigned `CoAes.BuiltinRules.assumption
    ...
    if applications.isEmpty then
      throwTacticEx `CoAes.BuiltinRules.assumption goal "no matching assumption found"
    return ⟨applications⟩
  where
    tryHyp (goal : MVarId) (fvarId : FVarId) (md : TransparencyMode) :
      ...
      let (true, steps) ← tryExactFVarS goal fvarId md |>.run
        | return none
      let #[step] := steps
        | throwError "coaes: internal error in assumption: multiple steps"
