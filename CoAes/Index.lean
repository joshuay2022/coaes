...
public import CoAes.Index.Basic
public import CoAes.Index.RulePattern
public import CoAes.RulePattern
public import CoAes.Rule.Basic
...
def trace [ToString (Rule α)] (ri : Index α) (traceOpt : TraceOption) :
  ...
  if ! (← traceOpt.isEnabled) then
    return
  withConstCoAesTraceNode traceOpt (return "Indexed by target") do
    traceArray ri.byTarget.values
  withConstCoAesTraceNode traceOpt (return "Indexed by hypotheses") do
    traceArray ri.byHyp.values
  withConstCoAesTraceNode traceOpt (return "Unindexed") do
    traceArray $ PersistentHashSet.toArray ri.unindexed
  where
    traceArray (as : Array (Rule α)) : CoreM Unit :=
      as.map toString |>.qsortOrd.forM λ r => do coaes_trace![traceOpt] r
...
def applicableRules (ri : Index α) (goal : MVarId)
  ...
  withConstCoAesTraceNode .debug (return "rule selection") do
  ...
  coaes_trace[debug] "selected rules:{.joinSep (result.map (m!"{·.rule.name}") |>.toList) "\n"}"
  return result
where
