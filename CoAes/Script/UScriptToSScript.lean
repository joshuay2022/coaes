...
partial def orderedUScriptToSScript (uscript : UScript) (tacticState : TacticState) : CoreM SScript :=
  withCoAesTraceNode .script (λ e => return m!"{exceptEmoji e} Converting ordered unstructured script to structured script") do
  ...
  coaes_trace[script] "step tree:{indentD $ toMessageData stepTree}"
  let focusable := stepTree.focusableGoals
  let numSiblings := stepTree.numSiblings
  coaes_trace[script] "focusable goals: {focusable.toArray.map λ (mvarId, n) => (mvarId.name, n)}"
  ...
  go (focusable : Std.HashMap MVarId Nat) (numSiblings : Std.HashMap MVarId Nat)
    ...
    if let some step := uscript[start]? then
      ...
      let some siblings := numSiblings[step.preGoal]?
        | throwError "coaes: internal error while structuring script: unknown sibling count for goal {step.preGoal.name}"
      coaes_trace[script] "siblings: {siblings}"
      let innerStop? := focusable[step.preGoal]?
      coaes_trace[script] "focusable: {innerStop?.isSome}"
      coaes_trace[script] "visible goals: {tacticState.visibleGoals.map (·.goal.name)}"
      let some goalPos := tacticState.getVisibleGoalIndex? step.preGoal
        ...
        | return (.empty, tacticState)
      coaes_trace[script] "goal position: {goalPos}"
