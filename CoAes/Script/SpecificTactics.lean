...
public import CoAes.Util.Tactic
public import CoAes.Util.Tactic.Ext
public import CoAes.Util.Tactic.Unfold
public import CoAes.Util.Unfold
public import CoAes.Script.CtorNames
public import CoAes.Script.ScriptM
...
def unfold (usedDecls : Array Name) (coaesUnfold : Bool) : TacticBuilder := do
  let usedDecls := usedDecls.map mkIdent
  let tac ←
  if coaesUnfold then
    `(tactic | coaes_unfold $usedDecls:ident*)
  else
    `(tactic | unfold $usedDecls:ident*)
  return .unstructured tac

def unfoldAt (goal : MVarId) (fvarId : FVarId) (usedDecls : Array Name)
    (coaesUnfold : Bool) : TacticBuilder :=
  goal.withContext do
    let hypIdent := mkIdent (← goal.withContext $ fvarId.getUserName)
    let usedDecls := usedDecls.map mkIdent
    let tac ←
    if coaesUnfold then
      `(tactic| coaes_unfold $usedDecls:ident* at $hypIdent:ident)
    else
...
private def isGlobalSimpTheorem (thms : SimpTheorems) (simprocs : Simprocs) :
    Origin → MetaM Bool
  -- The `post` field must be matched explicitly: a `↓`-fired (pre-phase)
  -- simproc or theorem has `post := false`, and a pattern `.decl decl` would
  -- silently fill in the default `post := true` and fail to match it.
  | origin@(.decl decl _post inv) => do
    -- A reversed (`← foo`) rewrite is not part of default `simp` behaviour
    -- even if `foo` is a default simp theorem.
    if inv then
      return false
    return simprocs.simprocNames.contains decl ||
      -- Builtin simprocs (e.g. `existsAndEq`) are compiled into Lean and do
      -- not appear in the simproc extension's `simprocNames`.
      (← Simp.isBuiltinSimproc decl) ||
...
def unfoldManyTargetS (unfold? : Name → Option (Option Name)) (goal : MVarId) :
  ...
  recordScriptStep {
    preGoal := goal
    tacticBuilders :=
      #[TacticBuilder.unfold usedDecls (coaesUnfold := false),
        TacticBuilder.unfold usedDecls (coaesUnfold := true)]
    postGoals := #[postGoal]
    preState, postState
  }
...
def unfoldManyAtS (unfold? : Name → Option (Option Name)) (goal : MVarId)
  ...
  recordScriptStep {
    preGoal := goal
    tacticBuilders :=
      #[TacticBuilder.unfoldAt goal fvarId usedDecls (coaesUnfold := false),
        TacticBuilder.unfoldAt goal fvarId usedDecls (coaesUnfold := true)]
    postGoals := #[postGoal]
    preState, postState
  }
  return some (postGoal, usedDecls)
...
def unfoldManyStarS (goal : MVarId) (unfold? : Name → Option (Option Name))  :
    ScriptM (Option (MVarId × Array Name)) :=
  goal.withContext do
    let initialGoal := goal
    let mut goal := goal
    let mut usedDecls := #[]
    if let some (goal', used) ← unfoldManyTargetS unfold? goal then
      goal := goal'
      usedDecls := usedDecls ++ used
    for ldecl in (← goal.getDecl).lctx do
      if ldecl.isImplementationDetail then
        continue
      if let some (goal', used) ← unfoldManyAtS unfold? goal ldecl.fvarId then
        goal := goal'
        usedDecls := usedDecls ++ used
