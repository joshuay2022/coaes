...
public import CoAes.Util.EqualUpToIds
public import CoAes.Script.Tactic
public import CoAes.Script.TacticState
...
structure LazyStep where
  ...
  preGoal : MVarId
  /--
  A nonempty list of tactic builders. During script generation, CoAes tries to
...
def runFirstSuccessfulTacticBuilder (s : LazyStep) : MetaM Tactic :=
  withConstCoAesTraceNode .script (return m!"converting lazy step to step") do
  withPPAnalyze do
    ...
    coaes_trace[script] "fallback: {fallback}"
    return fallback
where
  tryTacticBuilder (b : TacticBuilder) : MetaM (Option Tactic) := do
    let tactic ← b
    withCoAesTraceNode .script (λ res => return m!"{exceptOptionEmoji res} {tactic}") do
