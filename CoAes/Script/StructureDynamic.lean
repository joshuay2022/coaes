...
public import CoAes.Script.UScript
public import CoAes.Script.UScriptToSScript
public import CoAes.Script.Util
public import CoAes.Script.SScript
...
partial def structureDynamicCore (preState : Meta.SavedState) (preGoal : MVarId)
  ...
  withCoAesTraceNode .script (λ r => return m!"{exceptOptionEmoji r} Dynamically structuring the script") do
    coaes_trace[script] "unstructured script:{indentD $ MessageData.joinSep (uscript.map toMessageData |>.toList) "\n"}"
...
where
  go (preState : Meta.SavedState) (preGoals : Array MVarId) :
    ...
    if h : 0 < preGoals.size then
      ...
      let result? ← withCoAesTraceNode .script (λ r => return m!"{exceptOptionEmoji r} Focusing main goal {firstGoal.name}") do
        coaes_trace[script] "goal: {firstGoal.name}{← preState.runMetaM' $ addMessageContext $ indentD firstGoal}"
      ...
      | none =>
        ...
        withCoAesTraceNode .script (λ r => return m!"{exceptOptionEmoji r} Applying step to chronologically first goal") do
  ...
  goStructured (preState : Meta.SavedState) (preGoals : Array MVarId)
    ...
    let (some (_, step)) := (← read).steps[firstPreGoal]
      | throwError "coaes: internal error while structuring the script: no step for goal {preGoal.name}:{indentD $ ← preState.runMetaM' $ addMessageContext $ toMessageData preGoal}"
  ...
  goUnstructured (preState : Meta.SavedState) (preGoals : Array MVarId) :
    ...
    let some (goalPos, goal, _, step) := firstStep?
      | throwError "coaes: internal error while structuring the script: no step for any of the visible goals{indentD $ ← goalsToMessageData preState preGoals}"
    coaes_trace[script] "goal: {goal.name}{← preState.runMetaM' $ addMessageContext $ indentD goal}"
  ...
  applyStepAndSolveRemaining (preState : Meta.SavedState)
    ...
    coaes_trace[script] "applying step:{indentD $ toMessageData step}"
    coaes_trace[script] "expected post goals:{indentD $ ← goalsToMessageData step.postState (step.postGoals.map (·.goal))}"
    let (postState, postGoals) ←
      try
        runTacticCapturingPostState step.uTactic preState [preGoal]
      catch e =>
        coaes_trace[script] "tactic failed with error:{indentD e.toMessageData}"
        return none
    let postGoals := postGoals.toArray
    withUpdatedMVarIds step.postState postState (step.postGoals.map (·.goal)) postGoals
      (onFailure := do
        coaes_trace[script] "post goals don't match; actual post goals:{indentD $ ← goalsToMessageData postState postGoals}"
        return none) do
      coaes_trace[script] "post goals match"
