...
partial def structureStaticCore (tacticState : TacticState) (script : UScript) :
    CoreM (UScript × Bool) :=
  withConstCoAesTraceNode .script (return m!"statically structuring the tactic script") do
  coaes_trace[script] "unstructured script:{indentD $ MessageData.joinSep (script.toList.map λ step => m!"{step}") "\n"}"
...
where
  go (tacticState : TacticState) : StaticStructureM (List Step × TacticState) :=
    ...
    if let some goal := tacticState.visibleGoals[0]? then
      ...
      coaes_trace[script] "rendering step:{indentD $ toMessageData step}"
  ...
  nextStep (tacticState : TacticState) (mainGoal : GoalWithMVars) :
    ...
    if mainGoal.mvars.isEmpty then
      let some (_, step) := steps[mainGoal.goal]?
        | throwError "coaes: internal error while structuring script: no script step for main goal {mainGoal.goal.name}"
      return step
    let firstStep? :=
      findFirstStep? tacticState.visibleGoals (steps[·.goal]?) (·.fst)
    let some (_, _, _, firstStep) := firstStep?
      | throwError "coaes: internal error while structuring script: no script step found for any of the goals {tacticState.visibleGoals.map (·.goal.name)}"
