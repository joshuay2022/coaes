...
public import CoAes.Script.StructureDynamic
public import CoAes.Script.StructureStatic
public import CoAes.Script.OptimizeSyntax
public import CoAes.Stats.Basic
public import CoAes.Options.Internal
...
def UScript.optimize (uscript : UScript) (proofHasMVar : Bool)
  ...
  let structureResult? ← do
    let opts ← getOptions
    if coaes.dev.dynamicStructuring.get opts &&
       ! (coaes.dev.optimizedDynamicStructuring.get opts && ! proofHasMVar) then
      structureDynamic
    else
      structureStatic
...
def checkAndTraceScript (uscript : UScript)
    ...
    (goal : MVarId) (options : CoAes.Options') (expectCompleteProof : Bool)
    (tacticName : String) :
    m Unit := do
