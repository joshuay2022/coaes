...
public import CoAes.Forward.Substitution
public import CoAes.RPINF
public import CoAes.Rule.Name
public import CoAes.Tracing
public import CoAes.Index.DiscrTreeConfig
...
def «elab» (stx : Term) (rule : Expr) : TermElabM RulePattern :=
  withConstCoAesTraceNode .debug (return m!"elaborating rule pattern") do
   -- TODO withNewMCtxDepth produces an error, but I don't understand why
  withLCtx {} {} $ withoutModifyingState do
    coaes_trace[debug] "rule: {rule}"
    coaes_trace[debug] "pattern: {stx}"
    let lmvarIds := collectLevelMVars {} (← instantiateMVars rule) |>.result
    coaes_trace[debug] "level metavariables in rule: {lmvarIds.map Level.mvar}"
    forallTelescope (← inferType rule) λ fvars _ => do
      ...
      coaes_trace[debug] "result: '{pat.expr}' with arg map{indentD $ toMessageData argMap}\nand level arg map{indentD $ toMessageData levelArgMap}"
