...
def rpinf (e : Expr) : BaseM RPINF :=
  withConstCoAesTraceNode .rpinf (return m!"rpinf") do
    coaes_trace[rpinf] "input:{indentExpr e}"
    let e ← rpinfRaw e
    let hash := pinfHash e.toExpr
    coaes_trace[rpinf] "result hash: {hash}"
    coaes_trace[rpinf] "resut:{indentExpr e.toExpr}"
    return { e with hash }
