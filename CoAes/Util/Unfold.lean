...
where
  ...
  -- multiple times.
  --
  -- CoAes calls `unfold` multiple times anyway, so the current implementation
  -- is slow but correct.
...
def unfoldManyAt (unfold? : Name → Option (Option Name)) (goal : MVarId)
  ...
  goal.withContext do
    ...
    let some (_, goal) ←
      ...
      | throwTacticEx `coaes_unfold goal "internal error: unexpected result of applySimpResultToLocalDecl"
    return some (goal, usedDecls)
