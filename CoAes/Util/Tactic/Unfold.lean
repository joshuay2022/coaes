...
public meta import CoAes.Util.Unfold
...
elab "coaes_unfold " ids:ident+ : tactic => do
  let toUnfold ← mkToUnfold ids
  liftMetaTactic λ goal => do
    match ← unfoldManyTarget (toUnfold[·]?) goal with
    | none => throwTacticEx `coaes_unfold goal "could not unfold any of the given constants"
    | some (goal, _) => return [goal]

elab "coaes_unfold " ids:ident+ " at " hyps:ident+ : tactic => do
  let toUnfold ← mkToUnfold ids
  liftMetaTactic λ goal => do
    let mut goal := goal
    for hypId in hyps do
      ...
      | none => throwTacticEx `coaes_unfold goal m!"could not unfold any of the given constants at hypothesis '{hypId}'"
