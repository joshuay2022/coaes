...
initialize registerTraceClass `CoAes.Util.EqualUpToIds
...
mutual
  ...
  unsafe def exprsEqualUpToIdsCore₂ (e₁ e₂ : Expr) : ExprsEqualUpToIdsM Bool :=
    withIncRecDepth do
    withTraceNodeBefore `CoAes.Util.EqualUpToIds (return m!"{← printExpr (← readMCtx₁) (← read).lctx₁ (← read).localInstances₁ e₁} ≟ {← printExpr (← readMCtx₂) (← read).lctx₂ (← read).localInstances₂ e₂}") do
      if ptrEq e₁ e₂ then
        trace[CoAes.Util.EqualUpToIds] "pointer-equal"
        return true
      else if ! e₁.hasMVar && ! e₂.hasMVar && e₁.equal e₂ then
        trace[CoAes.Util.EqualUpToIds] "structurally equal"
        return true
  ...
  unsafe def localContextsEqualUpToIdsCore (lctx₁ lctx₂ : LocalContext)
    ...
    if h : decls₁.size = decls₂.size then
      ...
      trace[CoAes.Util.EqualUpToIds] "number of hyps differs"
      return none
  where
    go (decls₁ decls₂ : Array LocalDecl) (h : decls₁.size = decls₂.size)
      ...
      if h' : i < decls₁.size then
        ...
        let eq ← withTraceNodeBefore `CoAes.Util.EqualUpToIds (return m!"comparing hyps {ldecl₁.userName}, {ldecl₂.userName}") do
  ...
  unsafe def unassignedMVarsEqualUpToIdsCore (mvarId₁ mvarId₂ : MVarId) :
      EqualUpToIdsM Bool :=
    withTraceNodeBefore `CoAes.Util.EqualUpToIds (return m!"comparing mvars {mvarId₁.name}, {mvarId₂.name}") do
      if let some result ← equalCommonMVars? mvarId₁ mvarId₂ then
        trace[CoAes.Util.EqualUpToIds] "common mvars are {if result then "identical" else "different"}"
        return result
      else if let some m₂ := (← get).equalMVarIds[mvarId₁]? then
        if mvarId₂ == m₂ then
          trace[CoAes.Util.EqualUpToIds] "mvars already known to be equal"
          return true
        else
          trace[CoAes.Util.EqualUpToIds] "mvar {mvarId₁.name} known to be equal to different mvar {m₂.name}"
          return false
      else
        let ctx ← read
        ...
        let some gctx := gctx?
          | return false
        withTraceNodeBefore `CoAes.Util.EqualUpToIds (return m!"comparing targets") do
