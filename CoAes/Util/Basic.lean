...
public import CoAes.Util.UnorderedArraySet
...
@[inline]
def time [Monad m] [MonadLiftT BaseIO m] (x : m α) : m (α × CoAes.Nanos) := do
...
@[inline]
def time' [Monad m] [MonadLiftT BaseIO m] (x : m Unit) : m CoAes.Nanos := do
  let start ← IO.monoNanosNow
  x
  let stop ← IO.monoNanosNow
  return ⟨stop - start⟩
...
def addTryThisTacticSeqSuggestion (ref : Syntax)
  ...
  if let some range := (origSpan?.getD ref).getRange? then
    ...
    let suggestion := {
      ...
      toCodeActionTitle? := some λ _ => "Replace coaes with the proof it found"
...
register_option coaes.smallErrorMessages : Bool := {
    defValue := false
    descr := "(coaes) Print smaller error messages. Used for testing."
  }
