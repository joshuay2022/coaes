...
def mkInitialTree (goal : MVarId) (rs : LocalRuleSet) : BaseM Tree := do
  ...
  let (forwardState, ms) ← withConstCoAesTraceNode .forward (return m!"building initial forward state") do
...
def getRootGoal : TreeM GoalRef := do
  ...
  if h : grefs.size = 1 then
    ...
    throwError "coaes: internal error: unexpected number of goals in root mvar cluster: {grefs.size}"
