...
public import CoAes.Tree.Traversal
public import CoAes.Tree.TreeM
public import CoAes.Util.UnionFind
...
def findPathForAssignedMVars (assignedMVars : UnorderedArraySet MVarId)
  ...
  if ! unseen.isEmpty then
    ...
    if unseen.any (! rootGoalMVars.contains ·) then
      ...
      throwError "coaes: internal error: introducing rapps not found for these mvars: {reallyUnseen}"
