...
/-
To extract a proof, we start in the `MetaM` state in which CoAes was called.
...
local macro "throwPRError " s:interpolatedStr(term) : term =>
  `(throwError m!"coaes: internal error during proof reconstruction: " ++ m!$s)
...
private partial def copyExprMVar (s : Meta.SavedState) (mvarId : MVarId) :
  ...
  unless ← mvarId.isDeclared do
    let (decl, depMVarIds) ← s.runMetaM' $ do
      ...
      coaes_trace[extraction] "declare ?{mvarId.name}:{indentD $ toMessageData mvarId}"
  ...
  | some (Sum.inl e) =>
    ...
    coaes_trace[extraction] "assign  ?{mvarId.name} := {toString e}"
    mvarId.assign e
  | some (Sum.inr d) =>
    for mvarId in ← getMVars (mkMVar d.mvarIdPending) do
      copyExprMVar s mvarId
    coaes_trace[extraction] "dassign ?{mvarId.name} := {d.fvars} => {d.mvarIdPending.name}"
...
private def visitGoal (parentEnv : Environment) (g : Goal) :
  ...
  coaes_trace[extraction] "visiting G{g.id}"
...
private def visitRapp (parentEnv : Environment) (parentGoal : MVarId) (r : Rapp) :
  ...
  coaes_trace[extraction] "visiting R{r.id}"
