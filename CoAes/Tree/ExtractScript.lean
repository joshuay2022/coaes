...
public import CoAes.Tree.TreeM
...
def visitGoal (g : Goal) : ExtractScriptM Unit := do
  withConstCoAesTraceNode .script (return m!"goal {g.id}") do
...
def visitRapp (r : Rapp) : ExtractScriptM Unit := do
  withConstCoAesTraceNode .script (return m!"rapp {r.id}") do
...
def extractScript : TreeM (UScript × Bool) :=
  withCoAesTraceNode .script (λ r => return m!"{exceptEmoji r} Extract script") do
  ...
  partial def GoalRef.extractSafePrefixScriptCore (gref : GoalRef) :
    ...
    if ! g.normalizationState.isProvenByNormalization then
      ...
      if safeRapps.size > 1 then
        throwError "coaes: internal error: goal {g.id} has {safeRapps.size} safe rapps"
      if let some rref := safeRapps[0]? then
        ...
        let some (postNormGoal, postNormState) := g.postNormGoalAndMetaState?
          | throwError "coaes: internal error at extractSafePrefixScript: goal {g.id} is not normalised"
...
def extractSafePrefixScript : TreeM (UScript × Bool) := do
  withCoAesTraceNode .script (λ r => return m!"{exceptEmoji r} Extract safe prefix script") do
