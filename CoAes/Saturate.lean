...
public import CoAes.Forward.State.Initial
...
def run (options : CoAes.Options') (x : SaturateM α) :
...
initialize
  registerTraceClass `coaes.saturate
...
where
  go (goal : MVarId) (fs : ForwardState) : SaturateM MVarId :=
    withIncRecDepth do
    trace[coaes.saturate] "goal {goal.name}:{indentD goal}"
  ...
  runRule {α} (goal : MVarId) (mvars : UnorderedArraySet MVarId)
    ...
    | .error exc =>
      trace[coaes.saturate] exc.toMessageData
      return none
    | .ok output =>
  ...
  go (hypDepths : Std.HashMap FVarId Nat) (fs : ForwardState) (queue : Queue)
    ...
    goal.withContext do
      if let some (m, queue) := queue.deleteMin then
        ...
        trace[coaes.saturate] "goal:{indentD goal}"
        let oldGoal := goal
        ...
        goal.withContext do
          ...
          trace[coaes.saturate] "added hyp (depth {depth}) {Expr.fvar hyp} : {type}"
...
def saturateMain (rs : LocalRuleSet) (goal : MVarId) (options : CoAes.Options') :
...
def saturate (rs : LocalRuleSet) (goal : MVarId) (options : CoAes.Options') :
    MetaM MVarId := do
