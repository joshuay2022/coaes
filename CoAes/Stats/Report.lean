...
public import CoAes.Stats.Extension
...
def default : StatsReport := λ statsArray => Id.run do
  ...
  f!"Statistics for {statsArray.size} CoAes calls in current and imported modules\n\
     Displaying totals and [averages]\n\
     Total CoAes time:      {fmtTime total samples}\n\
...
def scriptsCore (nSlowest := 30) (nontrivialOnly := false) :
  ...
  f!"Statistics for {statsArray.size} CoAes calls{if nontrivialOnly then f!" with nontrivial script generation" else ""} in current and imported modules\n\
     Total CoAes time:         {fmtTimes totalTimes}\n\
  ...
  f!"  - perfectly:            {perfectlyDynamicallyStructured}\n\
     \n\
     {nSlowest} CoAes calls with slowest script generation:\n\
