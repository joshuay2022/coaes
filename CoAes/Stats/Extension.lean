...
structure StatsExtensionEntry where
  /--
  The CoAes call for which stats were collected.
  -/
  coaesStx : Syntax
  /-
  The file in which CoAes was called.
  -/
  fileName : String
  /-
  The position in the file where CoAes was called.
  -/
...
def forCurrentFile [Monad m] [MonadLog m] (stx : Syntax) (stats : Stats) :
  ...
  return { coaesStx := stx, stats, fileName, position? }
...
def recordStatsForCurrentFileIfEnabled [Monad m] [MonadEnv m] [MonadOptions m]
    [MonadLog m] (coaesStx : Syntax) (stats : Stats) : m Unit := do
  if ← enableStatsCollection then
    let entry ← StatsExtensionEntry.forCurrentFile coaesStx stats
