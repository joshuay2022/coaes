...
/--
Search strategies which CoAes can use.
-/
inductive Strategy
  /--
  ...
  | bestFirst
  /--
  Depth-first search. Whenever a rule is applied, CoAes immediately tries to
  ...
  | depthFirst
  /--
  Breadth-first search. CoAes always works on the oldest unsolved goal. Goal and
...
/--
Options which modify the behaviour of the `coaes` tactic.
-/
structure Options where
  /--
  The search strategy used by CoAes.
  -/
  ...
  exceeded, normalisation is likely stuck in an infinite loop, so CoAes fails. 0
  means no limit.
  -/
  maxNormIterations := 100
  /--
  When CoAes fails to prove a goal, it reports the goals that remain after safe
  ...
  before the safe prefix has been completely generated. In this case, CoAes
  ...
  /--
  If `true`, CoAes succeeds only if it proves the goal. If `false`, CoAes always
  succeeds and reports the goals remaining after safe rules were applied.
  -/
  terminal := false
  /--
  If `true`, print a warning when CoAes does not prove the goal.
  This can also be turned off globally with the option `coaes.warn.nonterminal`.
  -/
  warnOnNonterminal := true
  /--
  If CoAes proves a goal and this option is `true`, CoAes prints a tactic proof
  ...
  `@[simp]`. If this option is `false`, CoAes uses only CoAes-specific simp
  theorems, i.e. those tagged with `@[coaes simp]`. Note that the congruence
  ...
  enableUnfold := true
  /--
  Enable the builtin closing rules (`simp`, `simp_all`, `omega`, `decide`,
  `grind`). These are tried on every goal before any other rule except
...
/--
(coaes) Only for use by CoAes developers. Enables dynamic script structuring.
-/
register_option coaes.dev.dynamicStructuring : Bool := {
  descr := "(coaes) Only for use by CoAes developers. Enables dynamic script structuring."
  defValue := true
}

/--
(coaes) Only for use by CoAes developers. Uses static structuring instead of
dynamic structuring if no metavariables appear in the proof.
-/
register_option coaes.dev.optimizedDynamicStructuring : Bool := {
  descr := "(coaes) Only for use by CoAes developers. Uses static structuring instead of dynamic structuring if no metavariables appear in the proof."
  defValue := true
}

/--
(coaes) Only for use by CoAes developers. Generates a script even if none was requested.
-/
register_option coaes.dev.generateScript : Bool := {
  descr := "(coaes) Only for use by CoAes developers. Generates a script even if none was requested."
  defValue := false
}

/--
(coaes) Only for use by CoAes developers. Enables the new stateful forward reasoning engine.
-/
register_option coaes.dev.statefulForward : Bool := {
  descr := "(coaes) Only for use by CoAes developers. Enables the new stateful forward reasoning engine."
  defValue := true
}

/--
(coaes) Warn when apply builder is applied to a rule with conclusion of the form A ↔ B.
-/
register_option coaes.warn.applyIff : Bool := {
  descr := "(coaes) Warn when apply builder is applied to a rule with conclusion of the form A ↔ B."
  defValue := true
}

/--
(coaes) Warn when `coaes` does not close the goal, i.e. is used as a non-terminal tactic.
-/
register_option coaes.warn.nonterminal : Bool := {
  descr := "(coaes) Warn when `coaes` does not close the goal, i.e. is used as a non-terminal tactic."
  defValue := true
}

/--
(coaes) Collect statistics about CoAes invocations.
-/
register_option coaes.collectStats : Bool := {
  descr := "(coaes) Collect statistics about CoAes invocations. Use #coaes_stats to display the collected statistics."
  defValue := false
}

/--
(coaes) Write statistics about CoAes invocations to the given file in JSONL format.
-/
register_option coaes.stats.file : String := {
  descr := "(coaes) Write statistics about CoAes invocations to the given file in JSONL format. Each invocation adds one JSON record to the file."
  defValue := ""
}
