...
public import CoAes.Util.Basic
...
def registerTraceOption (traceName : Name) (descr : String) :
    IO TraceOption := do
  let option ← Option.register (`trace.coaes ++ traceName) {
    defValue := false
    descr
  }
  return { traceClass := `coaes ++ traceName, option }
...
initialize steps : TraceOption ←
  registerTraceOption .anonymous
    "(coaes) Print actions taken by CoAes during the proof search."

initialize ruleSet : TraceOption ←
  registerTraceOption `ruleSet
    "(coaes) Print the rule set before starting the search."

initialize proof : TraceOption ←
  registerTraceOption `proof
    "(coaes) If the search is successful, print the produced proof term."

initialize tree : TraceOption ←
  registerTraceOption `tree
    "(coaes) Once the search has concluded (successfully or unsuccessfully), print the final search tree."

initialize extraction : TraceOption ←
  registerTraceOption `extraction
    "(coaes) Print a trace of the proof extraction procedure."

initialize stats : TraceOption ←
  registerTraceOption `stats
    "(coaes) If the search is successful, print some statistics."

initialize debug : TraceOption ←
  registerTraceOption `debug
    "(coaes) Print various debugging information."

initialize script : TraceOption ←
  registerTraceOption `script
    "(coaes) Print a trace of script generation."

initialize forward : TraceOption ←
  registerTraceOption `forward
    "(coaes) Trace forward reasoning."

initialize forwardDebug : TraceOption ←
  registerTraceOption `forward.debug
    "(coaes) Trace more information about forward reasoning. Mostly intended for performance analysis."

initialize rpinf : TraceOption ←
  registerTraceOption `rpinf
    "(coaes) Trace RPINF calculations."
...
macro "coaes_trace![" opt:ident "] " msg:(interpolatedStr(term) <|> term) :
    doElem => do
  ...
  let msg ← if msg.getKind == interpolatedStrKind then
    ...
    `(toMessageData ($(⟨msg⟩)))
  `(doElem| Lean.addTrace (CoAes.TraceOption.traceClass $opt) $msg)

macro "coaes_trace[" opt:ident "] "
  ...
  | `(do $action) =>
    `(doElem| do
        if ← CoAes.TraceOption.isEnabled $opt then
          $action)
  | _ =>
    `(doElem| do
        if ← CoAes.TraceOption.isEnabled $opt then
...
def withCoAesTraceNode (opt : TraceOption)
    (msg : Except ε α → m MessageData) (k : m α) (collapsed := true) : m α :=
  withTraceNode opt.traceClass msg k collapsed

@[inline, always_inline]
def withCoAesTraceNodeBefore [ExceptToEmoji ε α] (opt : TraceOption)
    (msg : m MessageData) (k : m α) (collapsed := true) : m α :=
  withTraceNodeBefore opt.traceClass msg k collapsed

@[inline, always_inline]
def withConstCoAesTraceNode (opt : TraceOption) (msg : m MessageData) (k : m α)
    (collapsed := true) : m α :=
  withCoAesTraceNode opt (λ _ => msg) k collapsed
...
def traceSimpTheorems (s : SimpTheorems) (opt : TraceOption) : CoreM Unit := do
  if ! (← opt.isEnabled) then
    return
  withConstCoAesTraceNode opt (return "Erased entries") do
    coaes_trace![opt] "(Note: even if these entries appear in the sections below, they will not be used by simp.)"
    for e in PersistentHashSet.toArray s.erased |>.map (toString ·.key) |>.qsortOrd do
      coaes_trace![opt] e
  withConstCoAesTraceNode opt (return "Pre lemmas") do
    traceSimpTheoremTreeContents s.pre opt
  withConstCoAesTraceNode opt (return "Post lemmas") do
    traceSimpTheoremTreeContents s.post opt
  withConstCoAesTraceNode opt (return "Constants to unfold") do
